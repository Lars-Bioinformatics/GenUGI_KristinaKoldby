...
Rule all
'''
print(expand(output_somatic+"{sample}_tagseq-medexome_somatic_mutect2_filtered.vcf", sample=SAMPLES))
# sys.exit()

rule all_pairs:
    input:
        expand(output_somatic+"{sample}_tagseq-medexome_somatic_mutect2_filtered.vcf", sample=SAMPLES),
        # expand(output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.bam", sample=SAMPLES)
        # expand(output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf", sample=SAMPLES),
...
rule Mutect2_matched:
    input:
        # normal=config["{sample}"]["normal"]+"_tagseq-medexome.connor.recalibrated.bam",
        # tumor="{tumor}_tagseq-medexome.connor.recalibrated.bam",
        normal=expand("{normal}_tagseq-medexome.connor.recalibrated.bam", normal=NORMALS),
        tumors=expand("{tumor}_tagseq-medexome.connor.recalibrated.bam", tumor=TUMORS),
        pon=pon_location+"pon.vcf.gz",
        intervals=target_regions+"{interval}.interval_list"
    output:
        vcf=output_somatic+"split/{sample}_tagseq-medexome_somatic_mutect2__{interval}__split.vcf",
        idx=output_somatic+"split/{sample}_tagseq-medexome_somatic_mutect2__{interval}__split.vcf.idx",
        vcf_stats=output_somatic+"split/{sample}_tagseq-medexome_somatic_mutect2__{interval}__split.vcf.stats",
    ...
    params:
        tumors=expand("-I {tumor}_tagseq-medexome.connor.recalibrated.bam", tumor=TUMORS),
        normal_name=expand("-normal {normal}_tagseq-medexome", normal=NORMALS)
    threads: 24
    shell:
        """
...
rule merge_somatic_vcf:
    input:
        vcf_subfile=expand(output_somatic+"split/{{sample}}_tagseq-medexome_somatic_mutect2__{interval}__split.vcf", interval=INTERVALS)
    output:
        vcf=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf",
        idx=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf.idx",
    params:
        vcf_subfile=expand("-I "+output_somatic+"split/{{sample}}_tagseq-medexome_somatic_mutect2__{interval}__split.vcf", interval=INTERVALS)
    shell:
        """
...
rule merge_mutect2_bam:
  ...
  output:
        bam=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.bam"
  params:
...
rule merge_somatic_vcf_stats:
    input:
        vcf_subfile=expand(output_somatic+"split/{{sample}}_tagseq-medexome_somatic_mutect2__{interval}__split.vcf.stats", interval=INTERVALS)
    output:
        vcf_stats=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf.stats",
    params:
        vcf_subfile=expand("-stats "+output_somatic+"split/{{sample}}_tagseq-medexome_somatic_mutect2__{interval}__split.vcf.stats", interval=INTERVALS)
    shell:
        """
...
rule GetPileupSummaries:
    input:
        bam="{sample}_tagseq-medexome.connor.recalibrated.bam"
    output:
...
#         vcf=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf",
#         idx=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf.idx",
#         stats=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf.stats",
#         vcf_stats=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2.vcf.stats",
...
#     output:
#         vcf=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2_filtered.vcf",
#         idx=output_somatic+"{sample}_tagseq-medexome_somatic_mutect2_filtered.vcf.idx"
#     shell:
#         """
