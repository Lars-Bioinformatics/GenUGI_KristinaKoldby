...
rule all:
    input:
        expand("vaf_log2r_plots/plot_tables/{sample}_somatic_mutect2_log2r_af_filtered_ADmin4_DPmin50_NY.txt", sample=SAMPLES)
...
rule filter_vcf:
    input:
        "vaf_log2r_plots/{sample}_somatic_mutect2_log2r.vcf"
    output:
        "vaf_log2r_plots/{sample}_somatic_mutect2_log2r_filtered_ADmin4_DPmin50_NY.vcf"
    shell:
        """
        bcftools filter \
        --include 'FORMAT/AD[0:1] > 3' \
        {input} \
        -o {output}
        """
...
rule passed_variants:
    input:
        "vaf_log2r_plots/{sample}_somatic_mutect2_log2r_filtered_ADmin4_DPmin50_NY.vcf"
    output:
        "vaf_log2r_plots/{sample}_somatic_mutect2_log2r_filtered_ADmin4_DPmin50_passed_NY.txt"
    shell:
        """
        awk \
        -v FS='\t' \
...
rule final_plot_file:
    ...
    output:
        "vaf_log2r_plots/plot_tables/{sample}_somatic_mutect2_log2r_af_filtered_ADmin4_DPmin50_NY.txt"
    shell:
        """
