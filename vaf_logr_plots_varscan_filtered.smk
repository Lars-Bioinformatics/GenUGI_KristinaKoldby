...
rule all:
    input:
        # expand("vaf_log2r_plots/{sample}_varscan_filtered-NormADmax1-TumorADmin10-NormDPmin20.vcf.gz", sample=SAMPLES)
        ...
        # [expand("vaf_log2r_plots/{sample_subfile}_single_varscan_filtered-NormADmax1-TumorADmin10-NormDPmin20.vcf", sample_subfile=config[sample]["tumors"]) for sample in SAMPLES]
        # [expand("vaf_log2r_plots/{sample_subfile}_somatic_varscan_filtered_NormADmax1-TumorADmin10-NormDPmin20_log2r.vcf", sample_subfile=config[sample]["tumors"]) for sample in SAMPLES]
        # [expand("vaf_log2r_plots/{sample_subfile}_vs_{normal}_somatic_varscan_filtered_NormADmax1-TumorADmin10-NormDPmin20_log2r.vcf", sample_subfile=config[sample]["tumors"], normal=config[sample]["normal"]) for sample in SAMPLES]
        [expand("vaf_log2r_plots/plot_tables/{sample_subfile}_somatic_varscan_filtered_NormADmax1-TumorADmin10-NormDPmin20_log2r.txt", sample_subfile=config[sample]["tumors"]) for sample in SAMPLES]
...
rule filter_vcf:
    ...
    output:
        "vaf_log2r_plots/{sample}_varscan_filtered-NormADmax1-TumorADmin10-NormDPmin20.vcf.gz"
    shell:
        """
        bcftools filter \
        --include 'FORMAT/AD[*:0] >= 10 && FORMAT/AD[0:0] <= 1 && FILTER="PASS" && FORMAT/DP[0:0] >= 20' \
        {input} \
        -o {output}
        """
...
rule split_vcf:
    input:
        joint_vcf = lambda wildcards: expand("vaf_log2r_plots/{sample}_varscan_filtered-NormADmax1-TumorADmin10-NormDPmin20.vcf.gz", sample = '-'.join(wildcards.sample_subfile.split("-")[:2]))
    output:
        single_vcf = "vaf_log2r_plots/{sample_subfile}_single_varscan_filtered-NormADmax1-TumorADmin10-NormDPmin20.vcf"
    shell:
        """
        bcftools view  \
...
rule add_log2r_to_vcf:
    input:
        ...
        single_vcf = "vaf_log2r_plots/{sample_subfile}_single_varscan_filtered-NormADmax1-TumorADmin10-NormDPmin20.vcf"
    output:
        "vaf_log2r_plots/{sample_subfile}_somatic_varscan_filtered_NormADmax1-TumorADmin10-NormDPmin20_log2r.vcf"
    shell:
        """
        bcftools annotate \
        -a {input.ngcgh} \
...
rule create_plot_table:
    input:
        "vaf_log2r_plots/{sample_subfile}_somatic_varscan_filtered_NormADmax1-TumorADmin10-NormDPmin20_log2r.vcf"
    output:
        "vaf_log2r_plots/plot_tables/{sample_subfile}_somatic_varscan_filtered_NormADmax1-TumorADmin10-NormDPmin20_log2r.txt"
    shell:
        """
        bcftools query \
        -H \
