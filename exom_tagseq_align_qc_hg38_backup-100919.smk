...
#where {samplenumber} is for 1 to n samples is S1 to Sn according to SampleSheet.csv
    input:
        "{sampleid}_{samplenumber}_{protocol}_{flowcell}_R1_001.fastq.gz",
        "{sampleid}_{samplenumber}_{protocol}_{flowcell}_R2_001.fastq.gz"
    output:
        bam = "hg38/{sampleid}_{samplenumber}_{protocol}_{flowcell}.sorted.bam",
        bai = "hg38/{sampleid}_{samplenumber}_{protocol}_{flowcell}.sorted.bai"
    threads: 24
    log:
        "hg38/{sampleid}_{samplenumber}_{protocol}_{flowcell}.bwa_mem.log"
    params:
        rgid = "{sampleid}_{samplenumber}_{protocol}_{flowcell}",
        rglb = "{protocol}",
        rgsm = "{sampleid}_{samplenumber}_{protocol}_{flowcell}",
        rgpl = "illumina",
        rgpu = "{flowcell}"
    shell:
        """
