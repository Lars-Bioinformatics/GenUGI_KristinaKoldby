...
rule bwa_mem:
    ...
    output:
        bam = temp("hg38/{sampleid}_{protocol}_{flowcell}.sorted.bam"),
        bai = temp("hg38/{sampleid}_{protocol}_{flowcell}.sorted.bai")
    threads: 24
    log:
...
rule CollectWgsMetrics:
    input:
        ...
        bai = "hg38/{sample}.recalibrated.bai",
    output:
        "hg38/quality_control/{sample}_WgsMetrics.txt"
    shell:
        """
...
rule multiqc:
    input:
        ...
        expand("hg38/quality_control/{sample}_WgsMetrics.txt", sample=SAMPLES),
